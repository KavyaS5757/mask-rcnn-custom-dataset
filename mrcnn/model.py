...
def se_block(input_tensor, reduction_ratio=16):
    ...
    # Squeeze phase: Global Average Pooling
    x = KL.GlobalAveragePooling2D()(input_tensor)

    # Excitation phase: Fully connected layers
    x = KL.Dense(num_channels // reduction_ratio, activation='relu')(x)
    x = KL.Dense(num_channels, activation='sigmoid')(x)

    # Reshape to (batch_size, 1, 1, num_channels)
    x = KL.Reshape((1, 1, num_channels))(x)
...
def self_attention_block(input_tensor, name='self_attention'):
    ...
    attention_weights = KL.Conv2D(1, (1, 1), activation='sigmoid', name=name + '_weights')(input_tensor)
    
    # Apply attention weights to input feature maps
    x = KL.Multiply()([input_tensor, attention_weights])
    
    return x

def channel_attention(input_tensor, name='channel_attention'):
    num_channels = input_tensor.shape[-1]
    # Global average pooling
    avg_pool = KL.GlobalAveragePooling2D()(input_tensor)
    # Fully connected layers to compute attention logits
    attention_logits = KL.Dense(num_channels // 2, activation='relu')(avg_pool)
    attention_logits = KL.Dense(num_channels, activation='sigmoid', name=name + '_weights')(attention_logits)
    # Reshape the attention logits to be applied element-wise
    attention_logits = KL.Reshape((1, 1, num_channels))(attention_logits)
    # Apply attention weights to input feature maps
    x = KL.Multiply()([input_tensor, attention_logits])
    return x

def spatial_attention(input_tensor, name='spatial_attention'):
...
def identity_block(input_tensor, kernel_size, filters, stage, block,
    ...
    # Attention mechanism: Self-Attention block
    #x = self_attention_block(x, name='self_attention_' + str(stage) + block)
    
    # Attention mechanism: Channel Attention block
    #x = channel_attention(x, name='channel_attention_' + str(stage) + block)
    # Attention mechanism: Spatial Attention block
    #x = spatial_attention(x, name='spatial_attention_' + str(stage) + block)
...
def resnet_graph(input_image, architecture, stage5=False, train_bn=True):
    ...
    dropout_rate = 0.5
    # Add dropout layer after the Conv2D layer
    ...
    # Add SE block after the first Conv2D layer
    #x = se_block(x)
    # Attention mechanism: Self-Attention block
    #x = self_attention_block(x, name='self_attention_' + str(stage) + block)
...
class ProposalLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        anchors = inputs[2]
        # Backbone features
        backbone_features = inputs[3]
        
        # Apply attention mechanism (SE block) to the backbone features
        se_backbone_features = se_block(backbone_features)
        ...
        proposals = utils.batch_slice([boxes, scores], nms,
                                      self.config.IMAGES_PER_GPU)
        return proposals, se_backbone_features
...
def fpn_classifier_graph(rois, feature_maps, image_meta,
    ...
    # Apply SE block to the shared tensor
    shared_with_se = se_block(shared)

    # Classifier head
    mrcnn_class_logits = KL.TimeDistributed(KL.Dense(num_classes),
                                            name='mrcnn_class_logits')(shared_with_se)
    ...
    x = KL.TimeDistributed(KL.Dense(num_classes * 4, activation='linear'),
                           name='mrcnn_bbox_fc')(shared_with_se)
...
def build_fpn_mask_graph(rois, feature_maps, image_meta, pool_size, num_classes, train_bn=True,
                         fc_layers_size=1024, dropout_rate=0.2, kernel_regularizer=None, bias_regularizer=None):
    ...
    x = KL.TimeDistributed(KL.Conv2D(256, (3, 3), padding="same", kernel_regularizer=kernel_regularizer, bias_regularizer=bias_regularizer),
